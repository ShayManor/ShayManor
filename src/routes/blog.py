...
@blog.route("/add_email", methods=["POST"])
def add_email():
    try:
        data = request.get_json()
...
@blog.route("/remove_email/<email>", methods=["GET", "POST"])
...
def execute(recipient: str, subject: str, content: str):
    ...
    if os.path.exists(token_secret_file):
        creds = Credentials.from_authorized_user_file(token_secret_file, SCOPES)
...
@blog.route("/send_email", methods=["GET"])
def send_email():
    # Get emails
    SUPABASE_SERVICE_KEY = os.getenv("SUPABASE_SERVICE_KEY")
    SUPABASE_URL = os.getenv("SUPABASE_URL")
    ...
    # Get content
    blog_data = (
        supabase.table("site_blog").select("*").order("created_at", desc=True).execute()
    )
    ...
    try:
        for email in emails:
            execute(
                recipient=email,
                subject=get_title(subject),
                content=write_email(content, tldr, email),
            )
